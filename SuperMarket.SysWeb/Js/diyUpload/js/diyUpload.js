...
(function( $ ) {
    ...
    $.fn.extend({
		...
		*/
        diyUpload:function( opt, serverCallBack ) {
			...
			if (opt.buttonText && opt.buttonText!=null) {
			...
			webUploader.on('beforeFileQueued', function (file) {
			    if (webUploader.options.fileNumLimit <= $fileInput.next('.parentFileBox').find('.fileBoxUl>li').length) {
			        var liobj = $fileInput.next('.parentFileBox').find('.fileBoxUl>li:last-child');
			        var prefileid = $(liobj).attr("id").replace("fileBox_", "");
			        removeLi($fileInput, liobj, prefileid, webUploader);
			    } 
			});
			//绑定文件加入队列事件;
			...
			if (opt.imagedefaultpath && opt.imagedefaultpath.length > 0) {
			    for (var i = 0 ; i < opt.imagedefaultpath.length; i++) {
			        var imagepathobj = opt.imagedefaultpath[i];
			        if (imagepathobj.imagepath != null && imagepathobj.imagepath!="")
			        createBoxNew($fileInput, imagepathobj.imageid, imagepathobj.imagepath, webUploader)
	...
	//取消事件;	
	function removeLi($div, $li, file_id, webUploader) { 
	    if (webUploader.getFiles() > 0 || webUploader.getFile(file_id) != undefined) {
	    ...
	    var fieldvalue = $div.attr("bindfield"); 
        $("#ul" + fieldvalue + " li[liitemcode=" + file_id + "]").remove(); 
	}
	...
	function createBox( $fileInput, file, webUploader ) {
		...
		var $diyCancel = $fileBox.children('.diyCancel').one('click', function () {
		    removeLi($fileInput, $(this).parent('li'), file_id, webUploader );	
		});
		var $diySuccess = $fileBox.children('.diySuccess').one('click', function () {
		    removeLi($fileInput,$(this).parent('li'), file_id, webUploader);
		});
	...
	function createBoxNew($fileInput, fileid,filepath, webUploader) {
	    ...
	    if ($parentFileBox.find('.diyButton').length <= 0) {
	        ...
	        $cancelButton.bind('click', function () {
	            var fileArr = webUploader.getFiles('queued');
	            $.each(fileArr, function (i, v) {
	                removeLi($fileInput,$('#fileBox_' + v.id), v.id, webUploader);
	            });
	        });
	    ...
	    //生成预览缩略图;
	    $fileBox.find('.viewThumb').append('<img src="' + filepath + '" >');
	    $fileInput.next('.parentFileBox').children('.diyButton').remove();
	    var $fileBox = $('#fileBox_' + fileid);
	    var $diyBar = $fileBox.find('.diyBar');
	    $fileBox.removeClass('diyUploadHover');
	    $diyBar.fadeOut(1000, function () {
	        $fileBox.children('.diySuccess').show();
