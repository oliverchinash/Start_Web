...
namespace SuperMarket.Data.SysDB
{
	...
	/// </summary>
	public partial class GYProvinceDA: BaseSysDB
    {
        #region 实例化
		...
		{
		   string sql= @"insert into GYProvince( [Code],[Name],[FullName],[ParentCode],[IsActive],[PYFirst],[PYFull],Sort)VALUES
			            ( @Code,@Name,@FullName,@ParentCode,@IsActive,@PYFirst,@PYFull,@Sort);
		...
		public   int UpdateGYProvince(GYProvinceEntity entity)
		{
			string sql= @" UPDATE dbo.[GYProvince] SET
                       [Code]=@Code,[Name]=@Name,[FullName]=@FullName,[ParentCode]=@ParentCode,[IsActive]=@IsActive,[PYFirst]=@PYFirst,[PYFull]=@PYFull,[Sort]=@Sort
                       WHERE [Id]=@id";
		    DbCommand cmd = db.GetSqlStringCommand(sql);
	    ...
	    {
			string sql=@"delete from GYProvince where  and Id=@Id";
			DbCommand cmd = db.GetSqlStringCommand(sql);
		...
		{
			string sql= @"SELECT  [Id],[Code],[Name],[FullName],[ParentCode],[IsActive],[PYFirst],[PYFull],[DisableTime],Sort
							FROM
        ...
        /// <param name="db">数据库操作对象</param>
        /// <param name="columns">需要返回的列，不提供任何列名时默认将返回所有列</param>
        public GYProvinceEntity GetGYProvinceByCode(string code)
        {
            string sql = @"SELECT  [Id],[Code],[Name],[FullName],[ParentCode],[IsActive],[PYFirst],[PYFull],[DisableTime],Sort
							FROM
							dbo.[GYProvince] WITH(NOLOCK)	
        ...
        public   IList<GYProvinceEntity> GetGYProvinceList(int pagesize, int pageindex, ref  int recordCount )
		{
			string sql= @"SELECT   [Id],[Code],[Name],[FullName],[ParentCode],[IsActive],[PYFirst],[PYFull],[DisableTime],Sort
						...
						(SELECT ROW_NUMBER() OVER (ORDER BY Id desc) AS ROWNUMBER,
						 [Id],[Code],[Name],[FullName],[ParentCode],[IsActive],[PYFirst],[PYFull],[DisableTime],Sort from dbo.[GYProvince] WITH(NOLOCK)	
						WHERE  1=1 ) as temp 
        ...
        public IList<GYProvinceEntity> GetGYProvinceAll()
        {
            ...
            string sql = @"SELECT    [Id],[Code],[Name],[FullName],[ParentCode],[IsActive],[PYFirst],[PYFull],[DisableTime],Sort from dbo.[GYProvince] WITH(NOLOCK)	";
		    IList<GYProvinceEntity> entityList = new List<GYProvinceEntity>();
            DbCommand cmd = db.GetSqlStringCommand(sql); 
            using (IDataReader reader = db.ExecuteReader(cmd))
            {
