...
/*****************************************
功能描述：ConfigSmsProvider表的数据访问类。
创建时间：2017/4/18 14:25:37
创 建 人：jc001
变更记录：
******************************************/
...
{
	/// <summary>
	/// ConfigSmsProviderEntity的数据访问操作
	/// </summary>
	public partial class ConfigSmsProviderDA: BaseSuperMarketDB
    {
        #region 实例化
        public static ConfigSmsProviderDA Instance
        {
            get
            {
        ...
        class Nested
        {
            ...
            internal static readonly ConfigSmsProviderDA instance = new ConfigSmsProviderDA();
        }
        #endregion
		#region 代码生成
		 #region  自动产生
	    ...
	    {
			string sql=@"delete from ConfigSmsProvider where  and Id=@Id";
			DbCommand cmd = db.GetSqlStringCommand(sql);
        ...
        {
            string sql = @"delete from  ConfigSmsProvider  where  IsActive=0 and DisabledTime<DATEADD(month, -2, getDate())";
            DbCommand cmd = db.GetSqlStringCommand(sql); 
            return db.ExecuteNonQuery(cmd);
        }
		 /// <summary>
        ...
        {
            string sql = @"Delete from ConfigSmsProvider  where Id in (" + ids + ")";
            DbCommand cmd = db.GetSqlStringCommand(sql);
            return db.ExecuteNonQuery(cmd);
        }
		 /// <summary>
        ...
        {
            string sql = @"Update   ConfigSmsProvider set IsActive=0  where Id in ("+ ids + ")";
            DbCommand cmd = db.GetSqlStringCommand(sql); 
            return db.ExecuteNonQuery(cmd);
        }
		/// <summary>
