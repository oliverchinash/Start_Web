...
/*****************************************
功能描述：PartsProductFound表的数据访问类。
创建时间：2017/9/4 10:28:37
创 建 人：jc001
变更记录：
******************************************/
...
{
	/// <summary>
	/// PartsProductFoundEntity的数据访问操作
	/// </summary>
	public partial class PartsProductFoundDA: BaseSuperMarketDB
    {
        #region 实例化
        public static PartsProductFoundDA Instance
        {
            get
            {
        ...
        class Nested
        {
            ...
            internal static readonly PartsProductFoundDA instance = new PartsProductFoundDA();
        }
        #endregion
		#region 代码生成
		 #region  自动产生
...
IF not EXISTS ( SELECT  1
            FROM    PartsProductFound
            WHERE   [ProductName] = @ProductName ) 
    BEGIN
        INSERT  INTO PartsProductFound
                ( [ProductName] ,
                  [ProductFullName] ,
                  [PYFirst] ,
                  [PingYingFull] ,
...
ScopeId,ScopeType
        ...
        VALUES  ( @ProductName ,
                  @ProductName ,
                  dbo.fun_getPY(@ProductName) ,
                  dbo.f_GetPinyin(@ProductName) ,
                  1 ,
                  1,
@ScopeId,@ScopeType
                ) ;
...
";
            ...
            db.AddInParameter(cmd, "@ProductName", DbType.String, proname); 
            db.AddInParameter(cmd, "@ScopeId", DbType.Int32, _scopeid); 
            db.AddInParameter(cmd, "@ScopeType", DbType.Int32, _scopetype); 
            object identity = db.ExecuteScalar(cmd);
            if (identity == null || identity == DBNull.Value) return 0;
            return Convert.ToInt32(identity);
        }
	    ...
	    {
			string sql=@"delete from PartsProductFound where  and Id=@Id";
			DbCommand cmd = db.GetSqlStringCommand(sql);
        ...
        {
            string sql = @"delete from  PartsProductFound  where  IsActive=0 and DisabledTime<DATEADD(month, -2, getDate())";
            DbCommand cmd = db.GetSqlStringCommand(sql); 
            return db.ExecuteNonQuery(cmd);
        }
		 /// <summary>
        ...
        {
            string sql = @"Delete from PartsProductFound  where Id in (" + ids + ")";
            DbCommand cmd = db.GetSqlStringCommand(sql);
            return db.ExecuteNonQuery(cmd);
        }
		 /// <summary>
        ...
        {
            string sql = @"Update   PartsProductFound set IsActive=0  where Id in ("+ ids + ")";
            DbCommand cmd = db.GetSqlStringCommand(sql); 
            return db.ExecuteNonQuery(cmd);
        }
		/// <summary>
