...
{
	/// <summary>
	/// MemWZShopCartEntity的数据访问操作
	/// </summary>
	public partial class MemWZShopCartDA: BaseSuperMarketDB
    {
        #region 实例化
        public static MemWZShopCartDA Instance
        {
            get
            {
        ...
        class Nested
        {
            ...
            internal static readonly MemWZShopCartDA instance = new MemWZShopCartDA();
        }
        #endregion
		#region 代码生成
		 #region  自动产生
	    ...
	    {
			string sql=@"delete from MemWZShopCart where  and Id=@Id";
			DbCommand cmd = db.GetSqlStringCommand(sql);
        ...
        {
            string sql = @"delete from  MemWZShopCart  where  IsActive=0 and DisabledTime<DATEADD(month, -2, getDate())";
            DbCommand cmd = db.GetSqlStringCommand(sql); 
            return db.ExecuteNonQuery(cmd);
        }
		 /// <summary>
        ...
        {
            string sql = @"Delete from MemWZShopCart  where Id in (" + ids + ")";
            DbCommand cmd = db.GetSqlStringCommand(sql);
            return db.ExecuteNonQuery(cmd);
        }
		 /// <summary>
        ...
        {
            string sql = @"Update   MemWZShopCart set IsActive=0  where Id in ("+ ids + ")";
            DbCommand cmd = db.GetSqlStringCommand(sql); 
            return db.ExecuteNonQuery(cmd);
        }
		/// <summary>
