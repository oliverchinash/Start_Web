...
namespace SuperMarket.Data.MemberDB
{
    ...
    /// StoreEntity的数据访问操作
    /// </summary>
    public partial class StoreDA : BaseSuperMarketDB
    {
        #region 实例化
        public static StoreDA Instance
        {
            get
            {
        ...
        class Nested
        {
            ...
            internal static readonly StoreDA instance = new StoreDA();
        }
        #endregion
		#region 代码生成
		 #region  自动产生
		...
		public int AddStore(StoreEntity entity)
		{
		   string sql= @"
insert into Store( [MemId],[StoreName],[CompanyName],[LegalName],LegalMobilePhone,[LegalIdentityNo],[LegalIdentityPre],[LegalIdentityBeh],[MobilePhone],[ContactsManName],[Country],[ProvinceId],[CityId],[District],[Address],[Longitude],[Latitude],[LicenseNo],[LicensePath],[Status],[GradeLevel],[CreateTime])VALUES
        ...
        {
            string sql = @" UPDATE dbo.[Store] SET
        ...
        /// <param name="db">数据库操作对象</param>
        /// <param name="columns">需要返回的列，不提供任何列名时默认将返回所有列</param>
        public IList<StoreEntity> GetStoreList(int pagesize, int pageindex, ref int recordCount, string legalName, string companyName, int Status,string memcode,string contractname,string contractphone,int _issupplier)
        {
            ...
            string sql = @"SELECT   MemCode,[Id],[MemId],[StoreName],[CompanyName],[LegalName],[LegalMobilePhone],[LegalIdentityNo],[LegalIdentityPre],[LegalIdentityBeh],[MobilePhone],[ContactsManName],[Country],[ProvinceId],[CityId],[District],[Address],[Longitude],[Latitude],[LicenseNo],[LicensePath],[Status],[GradeLevel],[CreateTime]
						...
						(SELECT ROW_NUMBER() OVER (ORDER BY a.Id desc) AS ROWNUMBER,
						 b.MemCode,a.[Id],a.[MemId],a.[StoreName],a.[CompanyName],a.[LegalName],a.[LegalMobilePhone],a.[LegalIdentityNo],a.[LegalIdentityPre],a.[LegalIdentityBeh],a.[MobilePhone],a.[ContactsManName],a.[Country],a.[ProvinceId],a.[CityId],a.[District],a.[Address],a.[Longitude],a.[Latitude],a.[LicenseNo],a.[LicensePath],b.[Status],a.[GradeLevel],a.[CreateTime] from dbo.[Store] a WITH(NOLOCK)	
inner join dbo.Member b  WITH(NOLOCK) on a.MemId=b.Id 
						" + where + @") as temp 
						where rownumber BETWEEN ((@PageIndex - 1) * @PageSize + 1) AND @PageIndex * @PageSize";

            string sql2 = @"Select count(1) from dbo.[Store] a with (nolock) inner join dbo.Member b  WITH(NOLOCK) on a.MemId=b.Id  " + where;

            IList<StoreEntity> entityList = new List<StoreEntity>();
...
inner join dbo.Member b  WITH(NOLOCK) on a.MemId=b.Id  left join [MemFromBaidu] c  WITH(NOLOCK) on a.MemId=c.MemId
            ...
            string sql2 = @"Select count(1) from dbo.[Store] a with (nolock) 
inner join dbo.Member b  WITH(NOLOCK) on a.MemId=b.Id left join [MemFromBaidu] c  WITH(NOLOCK) on a.MemId=c.MemId " + where;
        ...
        public IList<UnitStr> ShowHasRegister(string str)
        {
            ...
            string sql = @"SELECT w.value1 FROM dbo.Member a WITH(NOLOCK)  INNER JOIN dbo.Store b WITH(NOLOCK)  ON a.status=1 AND a.id=b.Id INNER JOIN 
 ...
 from dbo.[Store] A WITH(NOLOCK) INNER JOIN dbo.Member B  WITH(NOLOCK) ON A.MemId=B.Id where MemId in (" + memids + ")" ;
        ...
        public VWShowStoreEntity GetNextStore(int preid)
        {
            ...
            string sql = @"  SELECT TOP 1 a.id as StoreId,StoreName, [CompanyName],a.CreateTime
 from dbo.[Store] A WITH(NOLOCK)  INNER JOIN  dbo.Member b ON  a.MemId=b.Id "+ where + " ORDER BY a.id desc ";
        ...
        /// <param name="db">数据库操作对象</param>
        /// <param name="columns">需要返回的列，不提供任何列名时默认将返回所有列</param>
        public IList<StoreEntity> GetStoreAll()
        {

            string sql = @"SELECT    [Id],[MemId],[StoreName],[CompanyName],[LegalName],LegalMobilePhone,[LegalIdentityNo],[LegalIdentityPre],[LegalIdentityBeh],[MobilePhone],[ContactsManName],[Country],[ProvinceId],[CityId],[District],[Address],[Longitude],[Latitude],[LicenseNo],[LicensePath],[Status],[GradeLevel],[CreateTime] from dbo.[Store] WITH(NOLOCK)	";
            IList<StoreEntity> entityList = new List<StoreEntity>();
            DbCommand cmd = db.GetSqlStringCommand(sql);
            using (IDataReader reader = db.ExecuteReader(cmd))
            {
        ...
        /// <returns></returns>
        public int ExistNum(StoreEntity entity)
        {
            ///id=0,判断总数，ID>0判断除自己之外的总数
            string sql = @"Select count(1) from dbo.[Store] WITH(NOLOCK) ";
            string where = "where  ";
            if (entity.Id == 0)
            {
        ...
        public int ExistNum(string name,string address)
        {
            ///id=0,判断总数，ID>0判断除自己之外的总数
            string sql = @"Select count(1) from dbo.[Store] WITH(NOLOCK) ";
            string where = "where StoreName=@StoreName and  Address=@Address ";
      ...
      ,[LegalIdentityBeh],c.[MobilePhone] AS StoreMobile,c.ContactsManName,[Country],[ProvinceId],[CityId]
  ...
  ON a.id=b.MemId LEFT JOIN dbo.Store c WITH(NOLOCK) ON a.id=c.MemId   	
        ...
        /// <param name="db">数据库操作对象</param>
        /// <param name="columns">需要返回的列，不提供任何列名时默认将返回所有列</param>
        public StoreEntity GetStoreByMemId(int memid)
        {
        ...
        public IList<StoreEntity> GetStoreByLegalName(int pagesize, int pageindex, ref int recordCount, string LegalName)
        {
            ...
            string sql1 = @"SELECT [Id],[MemId],[StoreName],[CompanyName],[LegalName],[LegalMobilePhone],[LegalIdentityNo],[LegalIdentityPre],[LegalIdentityBeh],[MobilePhone],[ContactsManName],[Country],[ProvinceId],[CityId],[District],[Address],[Longitude],[Latitude],[LicenseNo],[LicensePath],[Status],[GradeLevel],[CreateTime]
                          ...
                          [ProvinceId],[CityId],[District],[Address],[Longitude],[Latitude],[LicenseNo],[LicensePath],[Status],[GradeLevel],[CreateTime] from dbo.[Store] WITH(NOLOCK) where [LegalName] Like @LegalName) as temp where ROWNUMBER BETWEEN (( @PageIndex - 1 ) * @PageSize + 1 ) AND @PageSize * @PageIndex";

            string sql2 = @"Select count(1) from dbo.[Store] with(nolock)";
        ...
        public IList<StoreEntity> GetStoreByCompanyName(int pagesize, int pageindex, ref int recordCount, string CompanyName)
        {
            ...
            string sql1 = @"SELECT [Id],[MemId],[StoreName],[CompanyName],[LegalName],[LegalMobilePhone],[LegalIdentityNo],[LegalIdentityPre],[LegalIdentityBeh],[MobilePhone],[ContactsManName],[Country],[ProvinceId],[CityId],[District],[Address],[Longitude],[Latitude],[LicenseNo],[LicensePath],[Status],[GradeLevel],[CreateTime]
                          ...
                          (SELECT ROW_NUMBER() OVER (Order By Id desc) AS ROWNUMBER,
                          [Id],[MemId],[StoreName],[CompanyName],[LegalName],[LegalMobilePhone],[LegalIdentityNo],[LegalIdentityPre],[LegalIdentityBeh],[MobilePhone],[ContactsManName],[Country],
                          [ProvinceId],[CityId],[District],[Address],[Longitude],[Latitude],[LicenseNo],[LicensePath],[Status],[GradeLevel],[CreateTime] from dbo.[Store] WITH(NOLOCK) where [CompanyName] Like @CompanyName) as temp where ROWNUMBER BETWEEN (( @PageIndex - 1 ) * @PageSize + 1 ) AND @PageSize * @PageIndex";
        ...
        public IList<StoreEntity> GetStoreByStatus(int pagesize, int pageindex, ref int recordCount, int Status)
        {
            ...
            string sql1 = @"SELECT [Id],[MemId],[StoreName],[CompanyName],[LegalName],[LegalMobilePhone],[LegalIdentityNo],[LegalIdentityPre],[LegalIdentityBeh],[MobilePhone],[ContactsManName],[Country],[ProvinceId],[CityId],[District],[Address],[Longitude],[Latitude],[LicenseNo],[LicensePath],[Status],[GradeLevel],[CreateTime]
                          ...
                          (SELECT ROW_NUMBER() OVER (Order By Id desc) AS ROWNUMBER,
                          [Id],[MemId],[StoreName],[CompanyName],[LegalName],[LegalMobilePhone],[LegalIdentityNo],[LegalIdentityPre],[LegalIdentityBeh],[MobilePhone],[ContactsManName],[Country],
                          [ProvinceId],[CityId],[District],[Address],[Longitude],[Latitude],[LicenseNo],[LicensePath],[Status],[GradeLevel],[CreateTime] from dbo.[Store] WITH(NOLOCK) where [Status] Like @Status) as temp where ROWNUMBER BETWEEN (( @PageIndex - 1 ) * @PageSize + 1 ) AND @PageSize * @PageIndex";
        ...
        {
            string sql = @"SELECT COUNT(1) FROM DBO.Store WITH(NOLOCK)  WHERE MobilePhone=@MobilePhone AND STATUS<>4";
            DbCommand cmd = db.GetSqlStringCommand(sql);
