...
{
	/// <summary>
	/// StoreBusscopeEntity的数据访问操作
	/// </summary>
	public partial class StoreBusscopeDA: BaseSuperMarketDB
    {
        #region 实例化
        public static StoreBusscopeDA Instance
        {
            get
            {
        ...
        class Nested
        {
            ...
            internal static readonly StoreBusscopeDA instance = new StoreBusscopeDA();
        }
        #endregion
		#region 代码生成
		 #region  自动产生
		...
		public int AddStoreBusscope(StoreBusscopeEntity entity)
		{
		   string sql=@"insert into StoreBusscope( [MemId],[ClassId])VALUES
			            ( @MemId,@ClassId);
		...
		public   int UpdateStoreBusscope(StoreBusscopeEntity entity)
		{
	    ...
	    {
			string sql=@"delete from StoreBusscope where  and Id=@Id";
			DbCommand cmd = db.GetSqlStringCommand(sql);
        ...
        {
            string sql = @"delete from  StoreBusscope  where  IsActive=0 and DisabledTime<DATEADD(month, -2, getDate())";
            DbCommand cmd = db.GetSqlStringCommand(sql); 
            return db.ExecuteNonQuery(cmd);
        }
		 /// <summary>
        ...
        {
            string sql = @"Delete from StoreBusscope  where Id in (" + ids + ")";
            DbCommand cmd = db.GetSqlStringCommand(sql);
            return db.ExecuteNonQuery(cmd);
        }
		 /// <summary>
        ...
        {
            string sql = @"Update   StoreBusscope set IsActive=0  where Id in ("+ ids + ")";
            DbCommand cmd = db.GetSqlStringCommand(sql); 
            return db.ExecuteNonQuery(cmd);
        }
		/// <summary>
		...
		public   StoreBusscopeEntity GetStoreBusscope(int id)
		{
			...
			db.AddInParameter(cmd,"@Id", DbType.Int32,id);
    		StoreBusscopeEntity entity=new StoreBusscopeEntity();
		...
		/// <param name="db">数据库操作对象</param>
		/// <param name="columns">需要返回的列，不提供任何列名时默认将返回所有列</param>
		public   IList<StoreBusscopeEntity> GetStoreBusscopeList(int pagesize, int pageindex, ref  int recordCount )
		{
			string sql=@"SELECT   [Id],[MemId],[ClassId]
						FROM
            ...
            IList<StoreBusscopeEntity> entityList = new List< StoreBusscopeEntity>();
            DbCommand cmd = db.GetSqlStringCommand(sql);
        ...
        /// <param name="db">数据库操作对象</param>
        /// <param name="columns">需要返回的列，不提供任何列名时默认将返回所有列</param>
        public IList<StoreBusscopeEntity> GetStoreBusscopeAll()
        {
        ...
        /// <returns></returns>
        public int  ExistNum(StoreBusscopeEntity entity)
        {
