...
{
	/// <summary>
	///  MemSuggestDealLogEntity的数据访问操作
	/// </summary>
	public partial class  MemSuggestDealLogDA: BaseSuperMarketDB
    {
        #region 实例化
        public static  MemSuggestDealLogDA Instance
        {
            get
            {
        ...
        class Nested
        {
            ...
            internal static readonly  MemSuggestDealLogDA instance = new  MemSuggestDealLogDA();
        }
        #endregion
		#region 代码生成
		 #region  自动产生
		...
		public int AddSuggestDealLog(MemSuggestDealLogEntity entity)
		{
		   string sql=@"insert into  MemSuggestDealLog( [SuggestId],[DealManId],[DealContent],[DealTime])VALUES
		...
		public   int UpdateSuggestDealLog(MemSuggestDealLogEntity entity)
		{
	    ...
	    {
			string sql=@"delete from  MemSuggestDealLog where  and Id=@Id";
			DbCommand cmd = db.GetSqlStringCommand(sql);
        ...
        {
            string sql = @"delete from   MemSuggestDealLog  where  IsActive=0 and DisabledTime<DATEADD(month, -2, getDate())";
            DbCommand cmd = db.GetSqlStringCommand(sql); 
            return db.ExecuteNonQuery(cmd);
        }
		 /// <summary>
        ...
        {
            string sql = @"Delete from  MemSuggestDealLog  where Id in (" + ids + ")";
            DbCommand cmd = db.GetSqlStringCommand(sql);
            return db.ExecuteNonQuery(cmd);
        }
		 /// <summary>
        ...
        {
            string sql = @"Update    MemSuggestDealLog set IsActive=0  where Id in ("+ ids + ")";
            DbCommand cmd = db.GetSqlStringCommand(sql); 
            return db.ExecuteNonQuery(cmd);
        }
		/// <summary>
		...
		public    MemSuggestDealLogEntity GetSuggestDealLog(int id)
		{
			...
			db.AddInParameter(cmd,"@Id", DbType.Int32,id);
            MemSuggestDealLogEntity entity =new  MemSuggestDealLogEntity();
		...
		/// <param name="db">数据库操作对象</param>
		/// <param name="columns">需要返回的列，不提供任何列名时默认将返回所有列</param>
		public   IList<MemSuggestDealLogEntity> GetSuggestDealLogList(int pagesize, int pageindex, ref  int recordCount )
		{
			string sql=@"SELECT   [Id],[SuggestId],[DealManId],[DealContent],[DealTime]
						FROM
            ...
            IList<MemSuggestDealLogEntity> entityList = new List<  MemSuggestDealLogEntity>();
            DbCommand cmd = db.GetSqlStringCommand(sql);
        ...
        /// <param name="db">数据库操作对象</param>
        /// <param name="columns">需要返回的列，不提供任何列名时默认将返回所有列</param>
        public IList<MemSuggestDealLogEntity> GetSuggestDealLogAll()
        {
        ...
        /// <returns></returns>
        public int  ExistNum(MemSuggestDealLogEntity entity)
        {
