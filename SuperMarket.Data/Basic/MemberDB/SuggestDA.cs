...
{
	/// <summary>
	/// SuggestEntity的数据访问操作
	/// </summary>
	public partial class SuggestDA: BaseSuperMarketDB
    {
        #region 实例化
        public static SuggestDA Instance
        {
            get
            {
        ...
        class Nested
        {
            ...
            internal static readonly SuggestDA instance = new SuggestDA();
        }
        #endregion
		#region 代码生成
		 #region  自动产生
		...
		public int AddSuggest(SuggestEntity entity)
		{
		   string sql=@"insert into Suggest( [SuggestType],[Title],[MobilePhone],[ManName],[Email],[SuggestContent],[IpAddress],[ClientType],[Status],[CreateTime])VALUES
		...
		public   int UpdateSuggest(SuggestEntity entity)
		{
			string sql=@" UPDATE dbo.[Suggest] SET
	    ...
	    {
			string sql=@"delete from Suggest where  and Id=@Id";
			DbCommand cmd = db.GetSqlStringCommand(sql);
        ...
        {
            string sql = @"delete from  Suggest  where  IsActive=0 and DisabledTime<DATEADD(month, -2, getDate())";
            DbCommand cmd = db.GetSqlStringCommand(sql); 
            return db.ExecuteNonQuery(cmd);
        }
		 /// <summary>
        ...
        {
            string sql = @"Delete from Suggest  where Id in (" + ids + ")";
            DbCommand cmd = db.GetSqlStringCommand(sql);
            return db.ExecuteNonQuery(cmd);
        }
		 /// <summary>
        ...
        {
            string sql = @"Update   Suggest set IsActive=0  where Id in ("+ ids + ")";
            DbCommand cmd = db.GetSqlStringCommand(sql); 
            return db.ExecuteNonQuery(cmd);
        }
		/// <summary>
		...
		/// <param name="db">数据库操作对象</param>
		/// <param name="columns">需要返回的列，不提供任何列名时默认将返回所有列</param>
		public   IList<SuggestEntity> GetSuggestList(int pagesize, int pageindex, ref  int recordCount )
		{
			string sql=@"SELECT   [Id],[SuggestType],[Title],[MobilePhone],[ManName],[Email],[SuggestContent],[IpAddress],[ClientType],[Status],[CreateTime]
						FROM
						(SELECT ROW_NUMBER() OVER (ORDER BY Id desc) AS ROWNUMBER,
						 [Id],[SuggestType],[Title],[MobilePhone],[ManName],[Email],[SuggestContent],[IpAddress],[ClientType],[Status],[CreateTime] from dbo.[Suggest] WITH(NOLOCK)	
						WHERE  1=1 ) as temp 
						where rownumber BETWEEN ((@PageIndex - 1) * @PageSize + 1) AND @PageIndex * @PageSize";
			
			string sql2=@"Select count(1) from dbo.[Suggest] with (nolock) ";
            IList<SuggestEntity> entityList = new List< SuggestEntity>();
            DbCommand cmd = db.GetSqlStringCommand(sql);
        ...
        /// <param name="db">数据库操作对象</param>
        /// <param name="columns">需要返回的列，不提供任何列名时默认将返回所有列</param>
        public IList<SuggestEntity> GetSuggestAll()
        {

            string sql = @"SELECT    [Id],[SuggestType],[Title],[MobilePhone],[ManName],[Email],[SuggestContent],[IpAddress],[ClientType],[Status],[CreateTime] from dbo.[Suggest] WITH(NOLOCK)	";
		    IList<SuggestEntity> entityList = new List<SuggestEntity>();
            DbCommand cmd = db.GetSqlStringCommand(sql); 
            using (IDataReader reader = db.ExecuteReader(cmd))
            {
        ...
        /// <returns></returns>
        public int  ExistNum(SuggestEntity entity)
        {
            ///id=0,判断总数，ID>0判断除自己之外的总数
            string sql = @"Select count(1) from dbo.[Suggest] WITH(NOLOCK) ";
            string where = "where ";
            if (entity.Id == 0)
            {
