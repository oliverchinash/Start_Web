...
{
	/// <summary>
	/// MemPostAddressEntity的数据访问操作
	/// </summary>
	public partial class MemPostAddressDA: BaseSuperMarketDB
    {
        #region 实例化
        public static MemPostAddressDA Instance
        {
            get
            {
        ...
        class Nested
        {
            ...
            internal static readonly MemPostAddressDA instance = new MemPostAddressDA();
        }
        #endregion
		#region 代码生成
		 #region  自动产生
        ...
        {
            string sql = @"UPDATE dbo.[MemPostAddress] SET IsDefault=0 where  [MemId]=@MemId;UPDATE dbo.[MemPostAddress] SET IsDefault =1 where Id=@id and  [MemId]=@MemId;  ";
            DbCommand cmd = db.GetSqlStringCommand(sql);
            ...
            /// </summary>
        public  int  DeleteMemPostAddressByKey(int id, int memid)
	    {
			string sql= @"delete from MemPostAddress where  Id=@Id and  [MemId]=@MemId";
			DbCommand cmd = db.GetSqlStringCommand(sql);
		    db.AddInParameter(cmd,"@Id", DbType.Int32,id);
            db.AddInParameter(cmd, "@MemId", DbType.Int32, memid);
            return  db.ExecuteNonQuery(cmd);
        ...
        /// </summary>
        /// <returns></returns>
        public int DeleteMemPostAddressDisabled()
        {
            string sql = @"delete from  MemPostAddress  where  IsActive=0 and DisabledTime<DATEADD(month, -2, getDate())";
            DbCommand cmd = db.GetSqlStringCommand(sql); 
            return db.ExecuteNonQuery(cmd);
        }
		 /// <summary>
        ...
        /// <returns></returns>
        public int DeleteMemPostAddressByIds(string ids)
        {
            string sql = @"Delete from MemPostAddress  where Id in (" + ids + ")";
            DbCommand cmd = db.GetSqlStringCommand(sql);
            return db.ExecuteNonQuery(cmd);           
        }
		 /// <summary>
        ...
        /// <param name="ids"></param>
        /// <returns></returns>
        public int DisableMemPostAddressByIds(string ids)
        {
            string sql = @"Update   MemPostAddress set IsActive=0  where Id in ("+ ids + ")";
            DbCommand cmd = db.GetSqlStringCommand(sql); 
            return db.ExecuteNonQuery(cmd);
        }

        public int SetDefaultMemPostAddress(int id)
        {
            string sql = @"Update MemPostAddress set IsDefault=0 ;
            Update MemPostAddress set IsDefault=1 where id=@Id";
            DbCommand cmd = db.GetSqlStringCommand(sql);
            db.AddInParameter(cmd, "@Id", DbType.Int32, id);
            return db.ExecuteNonQuery(cmd);
        }
        ...
        public   MemPostAddressEntity GetMemPostAddress(int id)
		{
        ...
        /// <param name="db">数据库操作对象</param>
        /// <param name="columns">需要返回的列，不提供任何列名时默认将返回所有列</param>
        public IList<MemPostAddressEntity> GetMemPostAddressAll()
        {

            string sql = @"SELECT    [Id],[MemId],[AccepterName],[CtType],[CountryCode],[CountryName],[ProvinceId],[CityId],[District],[Address],[Email],[Telephone],[MobilePhone],[IsDefault],[Sort] from dbo.[MemPostAddress] WITH(NOLOCK)	";
		    IList<MemPostAddressEntity> entityList = new List<MemPostAddressEntity>();
            DbCommand cmd = db.GetSqlStringCommand(sql); 
            using (IDataReader reader = db.ExecuteReader(cmd))
            {
        ...
        {
            ///id=0,判断总数，ID>0判断除自己之外的总数
            string sql = @"Select count(1) from dbo.[MemPostAddress] WITH(NOLOCK) where MemId=@MemId "; 
            DbCommand cmd = db.GetSqlStringCommand(sql); 
            db.AddInParameter(cmd, "@MemId", DbType.Int32, memid); 
            object identity = db.ExecuteScalar(cmd);
            if (identity == null || identity == DBNull.Value) return 0;
