...
/*****************************************
功能描述：MobileMessage表的数据访问类。
创建时间：2016/11/15 15:31:18
创 建 人：jc001
变更记录：
******************************************/
...
{
	/// <summary>
	/// MobileMessageEntity的数据访问操作
	/// </summary>
	public partial class MobileMessageDA: BaseSuperMarketDB
    {
        #region 实例化
        public static MobileMessageDA Instance
        {
            get
            {
        ...
        class Nested
        {
            ...
            internal static readonly MobileMessageDA instance = new MobileMessageDA();
        }
        #endregion
		#region 代码生成
		 #region  自动产生
	    ...
	    {
			string sql=@"delete from MobileMessage where  and Id=@Id";
			DbCommand cmd = db.GetSqlStringCommand(sql);
        ...
        {
            string sql = @"delete from  MobileMessage  where  IsActive=0 and DisabledTime<DATEADD(month, -2, getDate())";
            DbCommand cmd = db.GetSqlStringCommand(sql); 
            return db.ExecuteNonQuery(cmd);
        }
		 /// <summary>
        ...
        {
            string sql = @"Delete from MobileMessage  where Id in (" + ids + ")";
            DbCommand cmd = db.GetSqlStringCommand(sql);
            return db.ExecuteNonQuery(cmd);
        }
		 /// <summary>
        ...
        {
            string sql = @"Update   MobileMessage set IsActive=0  where Id in ("+ ids + ")";
            DbCommand cmd = db.GetSqlStringCommand(sql); 
            return db.ExecuteNonQuery(cmd);
        }
		/// <summary>
        ...
        {
            ///id=0,判断总数，ID>0判断除自己之外的总数
            string sql = @"SELECT count(1) FROM dbo.MobileMessage  WITH(NOLOCK)  WHERE IpInt=@IpInt AND DATEDIFF(day, CreateTime, GETDATE())=0";
             
            DbCommand cmd = db.GetSqlStringCommand(sql);
            db.AddInParameter(cmd, "@IpInt", DbType.Int64, ipint); 
            object identity = db.ExecuteScalar(cmd);
            if (identity == null || identity == DBNull.Value) return 0;
            return Convert.ToInt32(identity);
        }
        ...
        {
            ///id=0,判断总数，ID>0判断除自己之外的总数
            string sql = @"SELECT max(CreateTime) as CreateTime FROM dbo.MobileMessage  WITH(NOLOCK)  WHERE IpInt=@IpInt AND DATEDIFF(day, CreateTime, GETDATE())=0";

            DbCommand cmd = db.GetSqlStringCommand(sql);
            db.AddInParameter(cmd, "@IpInt", DbType.Int64, ipint);
            object identity = db.ExecuteScalar(cmd);
            if (identity == null || identity == DBNull.Value) return null;
            return Convert.ToDateTime(identity);
        } 
