...
{
	/// <summary>
	/// InquiryProductEntity的数据访问操作
	/// </summary>
	public partial class InquiryProductDA: BaseSuperMarketDB
    {
        #region 实例化
        public static InquiryProductDA Instance
        {
            get
            {
        ...
        class Nested
        {
            ...
            internal static readonly InquiryProductDA instance = new InquiryProductDA();
        }
        #endregion
		#region 代码生成
		 #region  自动产生
        ...
        {
            string sql = @" UPDATE dbo.[InquiryProduct] SET
                    [ProductNum]=@ProductNum ,[Remark]=@Remark,[ProductUnitName]=@ProductUnitName
                       WHERE   [InquiryOrderCode]=@InquiryOrderCode and [ProductId]=@ProductId";
            DbCommand cmd = db.GetSqlStringCommand(sql);
             
            db.AddInParameter(cmd, "@InquiryOrderCode", DbType.String, code); 
            db.AddInParameter(cmd, "@ProductId", DbType.Int32, proid); 
            db.AddInParameter(cmd, "@ProductNum", DbType.Int32, pronum); 
            db.AddInParameter(cmd, "@ProductUnitName", DbType.String, unitname);
            db.AddInParameter(cmd, "@Remark", DbType.String, remark); 
            return db.ExecuteNonQuery(cmd);
        }

        public int UpdateInquiryProductByCG(string code, int proid, int pronum,string unitname, string remark, int cgmemid)
        {
            string sql = @" UPDATE dbo.[InquiryProduct] SET
                    [ProductNum]=@ProductNum ,[Remark]=@Remark,[ProductUnitName]=@ProductUnitName
                       WHERE   [InquiryOrderCode]=@InquiryOrderCode and [ProductId]=@ProductId and CreateManId=@CreateManId";
            DbCommand cmd = db.GetSqlStringCommand(sql);
	    ...
	    {
			string sql=@"delete from InquiryProduct where    Id=@Id";
			DbCommand cmd = db.GetSqlStringCommand(sql);
        ...
        {
            string sql = @"delete from InquiryProduct where InquiryOrderCode=@InquiryOrderCode and ProductId=@ProductId ";
            DbCommand cmd = db.GetSqlStringCommand(sql);
        ...
        {
            string sql = @"delete from InquiryProduct where InquiryOrderCode=@InquiryOrderCode and ProductId=@ProductId and CreateManId=@CreateManId
delete from InquiryProductSub where InquiryOrderCode=@InquiryOrderCode and InquiryProductId=@ProductId and CreateManId=@CreateManId
";
            DbCommand cmd = db.GetSqlStringCommand(sql);
            db.AddInParameter(cmd, "@InquiryOrderCode", DbType.String, ordercode);
            db.AddInParameter(cmd, "@ProductId", DbType.Int32, proid);
        ...
        {
            string sql = @"delete from  InquiryProduct  where  IsActive=0 and DisabledTime<DATEADD(month, -2, getDate())";
            DbCommand cmd = db.GetSqlStringCommand(sql); 
            return db.ExecuteNonQuery(cmd);
        }
		 /// <summary>
        ...
        {
            string sql = @"Delete from InquiryProduct  where Id in (" + ids + ")";
            DbCommand cmd = db.GetSqlStringCommand(sql);
            return db.ExecuteNonQuery(cmd);
        }
        public int DeleteInquiryProductById(string ordercode,int proid)
        {
            string sql = @"Delete from InquiryProduct  where  InquiryOrderCode=@InquiryOrderCode AND Id=@ProductId";
            DbCommand cmd = db.GetSqlStringCommand(sql);
            db.AddInParameter(cmd, "@InquiryOrderCode", DbType.String, ordercode);
            db.AddInParameter(cmd, "@ProductId", DbType.Int32, proid);
            return db.ExecuteNonQuery(cmd);
        ...
        {
            string sql = @"Update   InquiryProduct set IsActive=0  where Id in ("+ ids + ")";
            DbCommand cmd = db.GetSqlStringCommand(sql); 
            return db.ExecuteNonQuery(cmd);
        }
		/// <summary>
        ...
        public VWInquiryProductEntity GetVWInquiryProduct(int id)
        {
            string sql = @"SELECT  [Id],[InquiryOrderCode],[ProductCode],[ClassesId],[ClassesName],ProductId,[ProductName],[ProductNum],[ProductUnitName],[Remark]
							FROM
							dbo.[InquiryProduct] WITH(NOLOCK)	
            ...
            db.AddInParameter(cmd, "@Id", DbType.Int32, id);
            VWInquiryProductEntity entity = new VWInquiryProductEntity();
            using (IDataReader reader = db.ExecuteReader(cmd))
            {
                if (reader.Read())
                {
