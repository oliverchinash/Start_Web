...
{
	/// <summary>
	/// CmsContentEntity的数据访问操作
	/// </summary>
	public   class CmsContentDA: BaseSuperMarketDB
    {
        #region 实例化
        public static CmsContentDA Instance
        {
            get
            {
        ...
        class Nested
        {
            ...
            internal static readonly CmsContentDA instance = new CmsContentDA();
        }
        #endregion
		#region 代码生成
		 #region  自动产生
	    ...
	    {
			string sql=@"delete from CmsContent where  and Id=@Id";
			DbCommand cmd = db.GetSqlStringCommand(sql);
        ...
        {
            string sql = @"delete from  CmsContent  where  IsActive=0 and DisabledTime<DATEADD(month, -2, getDate())";
            DbCommand cmd = db.GetSqlStringCommand(sql); 
            return db.ExecuteNonQuery(cmd);
        }
		 /// <summary>
        ...
        {
            string sql = @"Delete from CmsContent  where Id in (" + ids + ")";
            DbCommand cmd = db.GetSqlStringCommand(sql);
            return db.ExecuteNonQuery(cmd);
        }
		 /// <summary>
        ...
        {
            string sql = @"Update   CmsContent set IsActive=0  where Id in ("+ ids + ")";
            DbCommand cmd = db.GetSqlStringCommand(sql); 
            return db.ExecuteNonQuery(cmd);
        }
		/// <summary>
