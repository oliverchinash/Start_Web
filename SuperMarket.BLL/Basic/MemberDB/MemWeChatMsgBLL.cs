using System;
using System.Data;

using System.Collections.Generic;
using SuperMarket.Model;
using SuperMarket.Data.MemberDB;
using System.Threading.Tasks;
using SuperMarket.Model.Cache;
using SuperMarket.Core;

/*****************************************
功能描述：表MemWeChatMsg的业务逻辑层。
创建时间：2017/8/16 17:20:43
创 建 人：jc001
变更记录：
******************************************/
namespace SuperMarket.BLL.MemberDB
{
	  
	/// <summary>
	/// 表MemWeChatMsg的业务逻辑层。
	/// </summary>
	public class MemWeChatMsgBLL
	{
	    #region 实例化
        public static MemWeChatMsgBLL Instance
        {
            get
            {
                return Nested.instance;
            }
        }

        class Nested
        {
            static Nested()
            {
            }
            internal static readonly MemWeChatMsgBLL instance = new MemWeChatMsgBLL();
        }
        #endregion
		/// <summary>
		/// 插入一条记录到表MemWeChatMsg，如果表中存在自增字段，则返回值为新记录的自增字段值，否则返回0。
		/// 该方法提供给界面等UI层调用
		/// </summary>
		/// <param name="memWeChatMsg">要添加的MemWeChatMsg数据实体对象</param>
		public   int AddMemWeChatMsg(MemWeChatMsgEntity memWeChatMsg)
		{
			  if (memWeChatMsg.Id > 0)
            {
                return UpdateMemWeChatMsg(memWeChatMsg);
            }
				  else if (string.IsNullOrEmpty(memWeChatMsg.NickName))
            {
                return (int)CommonStatus.ADD_Fail_Empty;
            }	 
          
            else if (MemWeChatMsgBLL.Instance.IsExist(memWeChatMsg))
            {
                return (int)CommonStatus.ADD_Fail_Exist;
            }
            else
            {
                return MemWeChatMsgDA.Instance.AddMemWeChatMsg(memWeChatMsg);
            }
	 	}

		/// <summary>
		/// 更新一条MemWeChatMsg记录。
		/// 该方法提供给界面等UI层调用
		/// </summary>
		/// <param name="memWeChatMsg">待更新的实体对象</param>
		/// <param name="columns">要更新的列名，不提供任何列名时默认将更新主键之外的所有列</param>
		public   int UpdateMemWeChatMsg(MemWeChatMsgEntity memWeChatMsg)
		{
			return MemWeChatMsgDA.Instance.UpdateMemWeChatMsg(memWeChatMsg);
		}
		 /// <summary>
        /// 根据主键值删除记录。如果数据库不存在这条数据将返回0
        /// </summary>
        public int DeleteMemWeChatMsgByKey(int id)
        {
            return MemWeChatMsgDA.Instance.DeleteMemWeChatMsgByKey(id);
        }
		 /// <summary>
        /// 删除失效记录，默认保留2个月
        /// </summary>
        /// <returns></returns>
        public int DeleteMemWeChatMsgDisabled()
        {
            return MemWeChatMsgDA.Instance.DeleteMemWeChatMsgDisabled();
        }
		 /// <summary>
        /// 做失效处理
        /// </summary>
        /// <param name="ids"></param>
        /// <returns></returns>
        public int DeleteMemWeChatMsgByIds(string ids)
        {
             int[] intArray;
            string[] strids = ids.Split(',');
            intArray = Array.ConvertAll<string, int>(strids, s => int.Parse(s));
            string idstr = String.Join(",", intArray); 
            return MemWeChatMsgDA.Instance.DeleteMemWeChatMsgByIds(idstr); 
        }
	    /// <summary>
        /// 做失效处理
        /// </summary>
        /// <param name="ids"></param>
        /// <returns></returns>
        public int DisableMemWeChatMsgByIds(string ids)
        {
            int[] intArray;
            string[] strids = ids.Split(',');
            intArray = Array.ConvertAll<string, int>(strids, s => int.Parse(s));
            string idstr = String.Join(",", intArray);
            return MemWeChatMsgDA.Instance.DisableMemWeChatMsgByIds(idstr);
        }
		/// <summary>
		/// 根据主键获取一个MemWeChatMsg实体记录。
		/// 该方法提供给其他实体的业务逻辑层（Logic）方法调用
		/// </summary>
		/// <returns>MemWeChatMsg实体</returns>
		/// <param name="columns">要返回的列</param>
		public   MemWeChatMsgEntity GetMemWeChatMsg(int id)
		{
			return MemWeChatMsgDA.Instance.GetMemWeChatMsg(id);			
		}
        public MemWeChatMsgEntity GetMsgByAppUnionId(string appid,string unionid)
        {
            return MemWeChatMsgDA.Instance.GetMsgByAppUnionId(appid, unionid);
        }
        public int WeChatLogin(MemWeChatMsgEntity entity)
        {
            return MemWeChatMsgDA.Instance.WeChatLogin(entity);

        }
        ///// <summary>
        ///// 获得数据列表
        ///// </summary>
        public IList<MemWeChatMsgEntity> GetMemWeChatMsgList(int pageSize, int pageIndex, ref  int recordCount,IList<ConditionUnit> wherelist)
        {
            return MemWeChatMsgDA.Instance.GetMemWeChatMsgList(pageSize, pageIndex, ref recordCount);
        }
		
		public async Task GetMemWeChatMsgAll()
        {
            await Task.Run(() =>
            {
                string _cachekey ="MemWeChatMsgListKey";// SysCacheKey.MemWeChatMsgListKey;
                object obj = MemCache.GetCache(_cachekey); ;
                if (obj == null)
                {
                    IList<MemWeChatMsgEntity> list = null;
                    list = MemWeChatMsgDA.Instance.GetMemWeChatMsgAll();
                    MemCache.AddCache(_cachekey, list);
                }
            });
        }
		/// <summary>
        /// 判断对象是否存在
        /// </summary>
        /// <param name="dicEnum"></param>
        /// <returns></returns>
        public bool IsExist(MemWeChatMsgEntity memWeChatMsg)
        {
            return MemWeChatMsgDA.Instance.ExistNum(memWeChatMsg)>0;
        }
		
	}
}

