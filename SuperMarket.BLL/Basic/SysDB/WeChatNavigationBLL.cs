using System;
using System.Data; 
using System.Collections.Generic;
using SuperMarket.Model;
using SuperMarket.Data.SysDB;
using System.Threading.Tasks;
using SuperMarket.Model.Cache;
using SuperMarket.Core;

/*****************************************
功能描述：表WeChatNavigation的业务逻辑层。
创建时间：2017/8/16 16:21:29
创 建 人：jc001
变更记录：
******************************************/
namespace SuperMarket.BLL.SysDB
{
	  
	/// <summary>
	/// 表WeChatNavigation的业务逻辑层。
	/// </summary>
	public class WeChatNavigationBLL
	{
	    #region 实例化
        public static WeChatNavigationBLL Instance
        {
            get
            {
                return Nested.instance;
            }
        }

        class Nested
        {
            static Nested()
            {
            }
            internal static readonly WeChatNavigationBLL instance = new WeChatNavigationBLL();
        }
        #endregion
		/// <summary>
		/// 插入一条记录到表WeChatNavigation，如果表中存在自增字段，则返回值为新记录的自增字段值，否则返回0。
		/// 该方法提供给界面等UI层调用
		/// </summary>
		/// <param name="weChatNavigation">要添加的WeChatNavigation数据实体对象</param>
		public   int AddWeChatNavigation(WeChatNavigationEntity weChatNavigation)
		{
		     return WeChatNavigationDA.Instance.AddWeChatNavigation(weChatNavigation);
          
	 	}

		/// <summary>
		/// 更新一条WeChatNavigation记录。
		/// 该方法提供给界面等UI层调用
		/// </summary>
		/// <param name="weChatNavigation">待更新的实体对象</param>
		/// <param name="columns">要更新的列名，不提供任何列名时默认将更新主键之外的所有列</param>
		public   int UpdateWeChatNavigation(WeChatNavigationEntity weChatNavigation)
		{
			return WeChatNavigationDA.Instance.UpdateWeChatNavigation(weChatNavigation);
		}
		 /// <summary>
        /// 根据主键值删除记录。如果数据库不存在这条数据将返回0
        /// </summary>
        public int DeleteWeChatNavigationByKey(int id)
        {
            return WeChatNavigationDA.Instance.DeleteWeChatNavigationByKey(id);
        }
		 /// <summary>
        /// 删除失效记录，默认保留2个月
        /// </summary>
        /// <returns></returns>
        public int DeleteWeChatNavigationDisabled()
        {
            return WeChatNavigationDA.Instance.DeleteWeChatNavigationDisabled();
        }
		 /// <summary>
        /// 做失效处理
        /// </summary>
        /// <param name="ids"></param>
        /// <returns></returns>
        public int DeleteWeChatNavigationByIds(string ids)
        {
             int[] intArray;
            string[] strids = ids.Split(',');
            intArray = Array.ConvertAll<string, int>(strids, s => int.Parse(s));
            string idstr = String.Join(",", intArray); 
            return WeChatNavigationDA.Instance.DeleteWeChatNavigationByIds(idstr); 
        }
	    /// <summary>
        /// 做失效处理
        /// </summary>
        /// <param name="ids"></param>
        /// <returns></returns>
        public int DisableWeChatNavigationByIds(string ids)
        {
            int[] intArray;
            string[] strids = ids.Split(',');
            intArray = Array.ConvertAll<string, int>(strids, s => int.Parse(s));
            string idstr = String.Join(",", intArray);
            return WeChatNavigationDA.Instance.DisableWeChatNavigationByIds(idstr);
        }
		/// <summary>
		/// 根据主键获取一个WeChatNavigation实体记录。
		/// 该方法提供给其他实体的业务逻辑层（Logic）方法调用
		/// </summary>
		/// <returns>WeChatNavigation实体</returns>
		/// <param name="columns">要返回的列</param>
		public   WeChatNavigationEntity GetWeChatNavigation(int id)
		{
			return WeChatNavigationDA.Instance.GetWeChatNavigation(id);			
		}
        public WeChatNavigationEntity GetNavigationByCode(int code)
        {
            return WeChatNavigationDA.Instance.GetNavigationByCode(code);
        }
        public WeChatNavigationEntity GetNavigationByUrl(string url)
        {
            return WeChatNavigationDA.Instance.GetNavigationByUrl(url);
        }
        public int GetIdByUrl(string url)
        {
            return WeChatNavigationDA.Instance.GetIdByUrl(url);
        }
        ///// <summary>
        ///// 获得数据列表
        ///// </summary>
        public IList<WeChatNavigationEntity> GetWeChatNavigationList(int pageSize, int pageIndex, ref  int recordCount,string key)
        {
            return WeChatNavigationDA.Instance.GetWeChatNavigationList(pageSize, pageIndex, ref recordCount, key);
        }
		
		public async Task GetWeChatNavigationAll()
        {
            await Task.Run(() =>
            {
                string _cachekey ="WeChatNavigationListKey";// SysCacheKey.WeChatNavigationListKey;
                object obj = MemCache.GetCache(_cachekey);
                if (obj == null)
                {
                    IList<WeChatNavigationEntity> list = null;
                    list = WeChatNavigationDA.Instance.GetWeChatNavigationAll();
                    MemCache.AddCache(_cachekey, list);
                }
            });
        }
		/// <summary>
        /// 判断对象是否存在
        /// </summary>
        /// <param name="dicEnum"></param>
        /// <returns></returns>
        public bool IsExist(WeChatNavigationEntity weChatNavigation)
        {
            return WeChatNavigationDA.Instance.ExistNum(weChatNavigation)>0;
        }
		
	}
}

